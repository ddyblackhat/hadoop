...
public class FlowMapreducePartition {
	...
	public static void main(String[] args) throws Exception {
		...
		/**
		 * 加入自定义分区定义: AreaPatitioner
		 */
		job.setPartitionerClass(AreaPartitioner.class);
		/**
		 * 设置reduce task的数量，要跟AreaPartitioner返回的partition个数匹配 如果reduce
		 * task的数量比partitioner中分组数多，就会产生多余的几个空文件 如果reduce
		 * task的数量比partitioner中分组数少，就会发生异常，因为有一些key没有对应reducetask接收 (如果reduce
		 * task的数量为1，也能正常运行，所有的key都会分给这一个reduce task) reduce task 或 map task
		 * 指的是，reuder和mapper在集群中运行的实例
		 */
